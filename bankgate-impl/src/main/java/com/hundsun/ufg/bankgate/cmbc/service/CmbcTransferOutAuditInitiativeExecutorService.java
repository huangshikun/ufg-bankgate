...
/**
 * 券商(主动方)发送“券商转银行审批结果” 给民生银行(被动方)
 */
@Service
@Slf4j
public class CmbcTransferOutAuditInitiativeExecutorService extends AbstractExecutorService<TransferOutAuditInitiativeRequest> {

	@Override
	public void sendMessage(TransferOutAuditInitiativeRequest bankRequest, BankResponse bankResponse) {
		CmbcRequest cmbcRequest = super.getCmbcRequest(bankRequest);
		cmbcRequest.setDstServ(EnumCmbcService.TRANSFER_OUT_AUDIT_INITIATIVE.getCode());
		try {
			String xml = JaxbUtil.beanToXml(cmbcRequest);
			log.info("券商(主动方)发送“券商转银行审批结果” 给民生银行(被动方)请求XML报文:[{}]", xml);
			...
			log.error("券商(主动方)发送“券商转银行审批结果” 给民生银行(被动方)异常", e);
		}
	}
}
