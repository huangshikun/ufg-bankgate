...
@Service
@Slf4j
public class CmbcAccountMatchExecutorService extends AbstractExecutorService<AccountMatchRequest> {

	@Override
	public void sendMessage(AccountMatchRequest bankRequest, BankResponse bankResponse) {
		CmbcRequest cmbcRequest = super.getCmbcRequest(bankRequest);
		cmbcRequest.setDstServ(EnumCmbcService.ACCOUNT_MATCH.getCode());
		try {
			String xml = JaxbUtil.beanToXml(cmbcRequest);
			log.info("民生银行协议对账请求XML报文:[{}]", xml);
