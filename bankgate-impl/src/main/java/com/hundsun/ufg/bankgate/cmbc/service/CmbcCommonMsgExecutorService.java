...
@Service
@Slf4j
public class CmbcCommonMsgExecutorService extends AbstractExecutorService<CommonMsgRequest> {

	@Override
	public void sendMessage(CommonMsgRequest bankRequest, BankResponse bankResponse) {
		CmbcRequest cmbcRequest = super.getCmbcRequest(bankRequest);
		cmbcRequest.setDstServ(EnumCmbcService.COMMON_MSG.getCode());
		try {
			String xml = JaxbUtil.beanToXml(cmbcRequest);
			log.info("民生银行通用消息返回请求XML报文:[{}]", xml);
